...
namespace fsystem {
    class ClearEnemy {
    public:
        void operator()(Registry &r, SparseArray<component::Position> &positions, SparseArray<component::Enemy> &enemies) const {
            Context context = r.getContext<Context>();

            for (auto &&[id, pos, enemy] : IndexedZipper(positions, enemies)) {
